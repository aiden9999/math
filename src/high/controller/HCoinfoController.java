...
@Controller
@RequestMapping("/high/community/info")
public class HCoinfoController {
	@Autowired
	HInfoService ns;
	...
	public ModelAndView write(){
		ModelAndView mav = new ModelAndView("/high/menu/community/info/write.jsp");
		mav.addObject("menu", "info");
		mav.addObject("topMenu", "community");
		return mav;
	}
	...
	public ModelAndView next(@PathVariable(name="num")int num){
		ModelAndView mav = new ModelAndView("/high/menu/community/info/infoAjax.jsp");
		List<HashMap> list = ns.infoList(num);
		mav.addObject("list", list);
		List<Integer> sep = ns.seperate();
		mav.addObject("sep", sep);
